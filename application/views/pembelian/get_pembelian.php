<form action="<?php echo site_url('/pembelian/item/'.$data[0]->id_pembelian) ?>" method="POST">
    ...
    <input type="hidden" name="id_pembelian" value="<?php echo $data[0]->id_pembelian;?>">
		<input type="hidden" name="id_detail_pembelian" value="<?php echo $data[0]->id_detail_pembelian;?>">
          ...
          <div class="form-group form-floating-label">
            <select name="id_item" class="form-control input-solid" id="selectFloatingLabel2" required>
              <?php 
                ...
                {
              ?>
                <option value="<?php echo $row->id_item;?>" <?php if($row->id_item == $data[0]->id_item){ echo 'selected'; }?>><?php echo $row->nama_item.' - '.$row->warna.' - '.$row->ukuran;?></option>
              <?php	} ?>
            </select>
      ...
      <div class="col-sm-12">
          <div class="form-group form-floating-label">
            <input value="<?php echo $data[0]->qty;?>" name="qty" autocomplete="off" id="inputFloatingLabel2" min="0" type="number" class="form-control input-solid" size="20" required>
